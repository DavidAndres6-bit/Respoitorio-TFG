...
public class DefectosController {
    ...
    private void accionSiguiente(ActionEvent event) {
        ...
        if (emisiones != null) {
            ...
            if (!opacidad.isEmpty() && !opacidad.equalsIgnoreCase("N/A")
                    || !coRalenti.isEmpty() && !coRalenti.equalsIgnoreCase("N/A")
                    || !coAcelerado.isEmpty() && !coAcelerado.equalsIgnoreCase("N/A")) {
                tieneEmisiones = true;
            }
        }
        ...
        if (!tieneEmisiones && !electrico) {
            Utilities.mostrarAlerta("Inspección Incompleta",
                    "No se han registrado los datos de emisiones. Por favor, vuelva a la sección de Emisiones antes de finalizar.",

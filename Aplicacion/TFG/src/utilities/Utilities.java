...
public class Utilities {
    ...
    private static final String REGEX_CONTRASENIA = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&._-])[A-Za-z\\d@$!%*?&._-]{8,}$";
    ...
    public static void mostrarAlerta(String titulo, String mensaje, Alert.AlertType tipo) {
        ...
        // Forzamos a que el texto se ajuste automáticamente en varias líneas si es largo
        alert.getDialogPane().setMinHeight(javafx.scene.layout.Region.USE_PREF_SIZE);
        
        // Le decimos al panel que calcule sus dimensiones según sus elementos internos
    ...
    */
    ...
    public static boolean dniValido(String dni) {
        // Variable para almacenar si el DNI es valido
        boolean esValido = false;

        // Primero comprobamos que no sea nulo y tenga la longitud correcta
        if (dni == null || dni.length() != 9) {
            esValido = false;
        } else {
            // Pasamos a mayúsculas para comparar la letra correctamente
            dni = dni.toUpperCase();

            // Validamos el formato (8 números y una letra válida)
            if (!dni.matches("^[0-9]{8}[TRWAGMYFPDXBNJZSQVHLCKE]$")) {
                esValido = false;
            } else {
                // Validamos la letra matemáticamente
                String letras = "TRWAGMYFPDXBNJZSQVHLCKE";
                int numero = Integer.parseInt(dni.substring(0, 8));
                char letraEsperada = letras.charAt(numero % 23);
                
                // La letra es el carácter en la posición 8 (el noveno)
                if (letraEsperada == dni.charAt(8)) {
                    esValido = true;
                } else {
                    esValido = false;
                }
    ...
    public static boolean contraseniaValida(String pass) {
        return pass != null && Pattern.matches(REGEX_CONTRASENIA, pass);
    }
    ...
    public static void configurarChoiceBox(ChoiceBox<String> cb, String codigoUnidad) {
        ...
        cb.getItems().clear();
        cb.getItems().addAll("SIN FALLOS", "LEVE (DL)", "GRAVE (DG)", "MUY GRAVE (DMG)");
        ...
        // Hemos preguntado a la IA como centrar el texto sin padding para que se vea bien con el texto de LEVE... y tambien el control de las ventanas emergentes
        ...
        cb.skinProperty().addListener((obs, oldSkin, newSkin) -> {
            if (newSkin != null) {
                ...
                if (label != null) {
                    ...
                    // Hacemos que la etiqueta interna ocupe casi todo el ancho para que el centrado sea real
            ...
            public void handle(ActionEvent event) {
                ...
                if (cb.isFocused()) {
                    ...
                    if (seleccion != null && !seleccion.equals("SIN FALLOS")) {
                        ...
                        dialog.setGraphic(null); // Quitamos el icono que viene por defecto
                        dialog.getDialogPane().setMinWidth(600);
                        dialog.getDialogPane().setStyle("-fx-font-size: 14pt; -fx-border-color: #bf820d; -fx-border-width: 2;");
                        ...
                        if (resultado.isPresent()) {
                            ...
                            // Si cancela o cierra el pop up sin hacer nada setteamos sin fallos en el spinner
